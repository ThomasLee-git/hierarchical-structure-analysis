/*
This is the code for the phrase-level analysis of the paper: Automatic Analysis
and Influence of Hierarchical Structure on Melody, Rhythm and Harmony in Popular
Music https://www.cs.cmu.edu/~rbd/papers/dai-mume2020.pdf

Input: Preprocessing results of a midi song from preprocessing.sh, including
melody.txt, finalized_chord.txt, analyzed_key.txt, time signature Output:
Phrase-level structure analysis result

Notice that it is an approximate version, if you want to run the full version,
turn off the timing code at line 1822 Author: Shuqi Dai Oct, 2020
*/
...
public:
  ...
  void printNote() const { printf(" (%d, %d) ", pitch, duration); }
};
...
public:
  ...
  int time_signature; // how many 16th beats are there in each bar, 4/4 - 16,
                      // 3/4 -12
  ...
  Section subsection(int l, int r) const {
    ...
    i = 0;
    while (i < chords.size() &&
           t + chords[i].duration < l * time_signature / 4) {
  ...
  bool is_bridge(int tip = 4) const {
    ...
    for (int i = 0; i < mel_len && t < time_len - tip; ++i) {
      if (melody[i].pitch > -2 && t + melody[i].duration > tip &&
          t < time_len - tip)
        return false;
      t += melody[i].duration;
    }
  ...
  bool is_subsec_bridge(int l, int r, int tip = 4) const {
    ...
    if (tmp_sec.is_bridge())
      return true;
    if (l > start) {
      Section tmp_sec = this->subsection(l - 1, r);
      int t = 0, i = 0;
      while (i < tmp_sec.melody.size()) {
        if (tmp_sec.melody[i].pitch > -2 && t < time_signature - 2 &&
            t + tmp_sec.melody[i].duration > time_signature) {
          int note_length = tmp_sec.melody[i++].duration;
          while (i < tmp_sec.melody.size() && tmp_sec.melody[i].pitch == -2)
            note_length += tmp_sec.melody[i++].duration;
          if (t + note_length >= time_signature * (r - l + 1) - tip)
            return true;
          else
            return false;
        }
  ...
  bool contains_bridge(int default_rest = 40) {
    int mel_len = melody.size(), t = 0, d,
        time_len = ((end - start) * time_signature);
    for (int i = 0; i < mel_len && t < time_len; ++i) {
      if (t + melody[i].duration > time_len)
        d = time_len - t;
      else
        d = melody[i].duration;
      if (melody[i].pitch == -2 && d >= default_rest)
        return true;
      t += d;
    }
    return false;
...
public:
  ...
  void insert_section(Section sec) {
    for (vector<Section>::iterator idx = sections.begin(); idx < sections.end();
         ++idx)
  ...
  bool is_same(const Match &x) {
    if (sections.size() != x.sections.size())
      return false;
    for (int i = 0; i < sections.size(); ++i)
      if (!sections[i].is_same(x.sections[i]))
        return false;
    return true;
  }
...
class Covermap {
public:
  unsigned long long map[3]; // cover status, bitmap map[0] 0 - 63, map[1] 64 -
                             // 127, map[2] 128 - 191
  int length;
  double cost;        // current SDL
  double match_score; // current composed rhythm similarity, contour similarity,
                      // chord similarity
  ...
  string string_result_merge_extra; // cover result after merging extra 'X'
                                    // measures
  ...
  bool is_same(Covermap x) {
    if (length != x.length)
      return 0;
    if (map[0] == x.map[0] && map[1] == x.map[1] && map[2] == x.map[2])
      return 1;
    return 0;
  }
  ...
  int bitonecount(const unsigned long long n) {
    return bitcount_table[n & 0xff] + bitcount_table[(n >> 8) & 0xff] +
           bitcount_table[(n >> 16) & 0xff] + bitcount_table[(n >> 24) & 0xff] +
           bitcount_table[(n >> 32) & 0xff] + bitcount_table[(n >> 40) & 0xff] +
           bitcount_table[(n >> 48) & 0xff] + bitcount_table[(n >> 56) & 0xff];
  }

  int getmap_at(int position) {
    if (position < 64)
      return int((map[0] >> position) & 1);
    if (position < 128)
      return int((map[1] >> (position - 64)) & 1);
  ...
  double best_possible_cost(double g, double h, int begin, int real_end) {
    ...
    int l = 0;
    if (begin < 64)
      l = bitonecount(map[0]) + bitonecount(map[1]) + bitonecount(map[2]) -
          bitonecount(map[0] & ((a << begin) - 1));
    else if (begin < 128)
      l = bitonecount(map[1]) + bitonecount(map[2]) -
          bitonecount(map[1] & ((a << (begin - 64)) - 1));
    else
      l = bitonecount(map[2]) -
          bitonecount(map[2] & ((a << (begin - 128)) - 1));
  ...
  void printMap() const {
    printf("Covermap: Length %d, c %.2lf, m %.2lf [ ", length, cost,
           match_score);
    for (int i = 0; i < length; ++i) {
      if (i < 64)
        printf("%d ", int((map[0] >> i) & 1));
      else if (i < 128)
        printf("%d ", int((map[1] >> (i - 64)) & 1));
      else
        printf("%d ", int((map[2] >> (i - 128)) & 1));
    }
    printf("], last cover ");
  ...
  void toString() {
    ...
    if (map[0] == 0 && map[1] == 0 && map[2] == 0) {
      string_result = "";
      if (real_start > 0)
        string_result += "i" + to_string(real_start);
      string_result += "X" + to_string(real_end - real_start);
      if (real_end < length)
        string_result += "o" + to_string(length - real_end);
      return;
    }
    Covermap *i = this;
    vector<Match> used;
    ...
    for (int j = 0; j < used.size(); ++j) {
      for (int k = 0; k < used[j].sections.size(); ++k) {
        ...
        for (int r = used[j].sections[k].start; r < used[j].sections[k].end;
             ++r)
          result_map[r] = j;
      }
    }
    ...
    for (int j = 0; j < ordered.sections.size(); ++j) {
      if (ordered.sections[j].start > t) {
        Section tmp_sub =
            original_song->subsection(t, ordered.sections[j].start);
        if (tmp_sub.is_bridge() ||
            original_song->is_subsec_bridge(t, ordered.sections[j].start)) {
          result += 'x';
          ...
          for (kr = 0; kr < tmp_sub.melody.size(); ++kr) {
            if (tmp_sub.melody[kr].duration >= 40 &&
                tmp_sub.melody[kr].pitch == -2)
              break;
            tmp_measure_count += tmp_sub.melody[kr].duration;
          }
          int bridge_start =
              tmp_measure_count / (original_song->time_signature) +
              int((tmp_measure_count % (original_song->time_signature)) > 4);
          int bridge_end =
              (tmp_measure_count + tmp_sub.melody[kr].duration) /
                  (original_song->time_signature) +
              int(((tmp_measure_count + tmp_sub.melody[kr].duration) %
                   (original_song->time_signature)) >
                  ((original_song->time_signature / 2)));
          if (bridge_start > 0) {
            result += 'X';
          ...
          int left_over = (ordered.sections[j].start - t) *
                              (original_song->time_signature) -
                          (tmp_measure_count + tmp_sub.melody[kr].duration);

          if (t + bridge_end + 1 == ordered.sections[j].start &&
              left_over <= (original_song->time_signature / 2))
          ...
          if (tmp_sub.melody[0].duration >= (original_song->time_signature) &&
              tmp_sub.melody[0].pitch == -2) {
            result += 'x';
            result += to_string(tmp_sub.melody[0].duration /
                                    (original_song->time_signature) +
                                int((tmp_sub.melody[0].duration %
                                     (original_song->time_signature)) >=
                                    (original_song->time_signature / 2)));
  ...
  int longest_note(vector<Note> melody, int tip = 16) {
    ...
    while (i >= 0 && pos <= tip) {
      if (melody[i].pitch == -2 && i > 0) {
        ...
        if (tmp > longest)
          longest = tmp;
      } else if (melody[i].duration > longest)
        longest = melody[i].duration;
      pos += melody[i--].duration;
    }
    return longest;
  }

  bool contain_match(const pair<Section, Section> candidate,
  ...
  void small_extra_merge() {
    if (string_result_merge_extra == "")
      string_result_merge_extra = string_result;
    if (string_result_merge_extra == "")
      return;
    int i = 0;
    ...
    while (i < string_result_merge_extra.length()) {
      ...
      int num = 0;
      while (i < string_result_merge_extra.length() &&
             string_result_merge_extra[i] >= '0' &&
             string_result_merge_extra[i] <= '9')
    ...
    for (int times = 0; times <= 1; ++times) {
      for (i = phrases.size() - 2; i >= 0; --i) {
        if (phrases[i].first >= 'A' && phrases[i].first <= 'Z' &&
            phrases[i].first != 'X' &&
            (phrases[i + 1].first == 'x' || phrases[i + 1].first == 'X') &&
            phrases[i + 1].second == 1) {
          bool flag1 =
              (i + 2 == phrases.size() ||
               (phrases[i + 2].first >= 'A' && phrases[i + 2].first <= 'Z'));
          bool flag2 = ((phrases[i].second + phrases[i + 1].second) % 4 == 0 ||
                        phrases[i + 1].first == 'x');
          if (flag1 && flag2) {
            ...
            for (int j = 0; j < phrases.size() && !delete_flag; ++j)
              if (phrases[i].first == phrases[j].first &&
                  phrases[i].second + 1 == phrases[j].second) {
                phrases[i].second += phrases[i + 1].second;
                phrases.erase(phrases.begin() + i + 1);
                delete_flag = true;
              }
  ...
  void merge_extra(const vector<pair<Section, Section>> &matches,
                   bool small_extra_only = false) {
    if (string_result == "")
      return;
    int i = 0;
    int current_pos = 0;
    ...
    while (i < string_result.length()) {
      ...
      if (label >= 'A' && label <= 'Z' && label != 'X' && label > max_label)
        max_label = label;
      int num = 0;
      while (i < string_result.length() && string_result[i] >= '0' &&
             string_result[i] <= '9')
        num = num * 10 + string_result[i++] - '0';

      current_pos += num;

      if ((prev_label >= 'A' && prev_label <= 'Z' &&
           (!small_extra_only || (small_extra_only && num <= 2))) &&
          (label == 'X' ||
           (label == 'x' &&
            !original_song->subsection(current_pos - num, current_pos)
                 .is_bridge()))) {
        ...
        for (int j = 0; j < new_result.size() - 1; ++j)
          if (new_result[j].first == prev_label) {
            Section tmp =
                (original_song->subsection(new_pos_end[j] - 1, new_pos_end[j]));
            int tmp_ln =
                longest_note(tmp.melody, original_song->time_signature);
            if (lowerbound_longest_note == -1 ||
                tmp_ln < lowerbound_longest_note)
              lowerbound_longest_note = tmp_ln;
          }
        if (lowerbound_longest_note >= 8 || lowerbound_longest_note == -1)
          lowerbound_longest_note = 8;
        int extra_lowerbound_longest_note = lowerbound_longest_note;
        if (extra_lowerbound_longest_note < 6)
          extra_lowerbound_longest_note = 6;
        bool flag0 = false, flag1 = false, flag2 = false;
        Section m0 = (original_song->subsection(current_pos - num - 1,
                                                current_pos - num));
        Section m1 = (original_song->subsection(current_pos - num,
                                                current_pos - num + 1));
        // Section m2 = (original_song->subsection(current_pos - num + 1,
        // current_pos - num + 2));//
        int ln0 = longest_note(m0.melody, original_song->time_signature);
        int ln1 = longest_note(m1.melody, original_song->time_signature),
            ln2 = 0;
        if (extra_lowerbound_longest_note < 8 &&
        ...
        if (num > 1) {
          ...
          if (ln2 >= extra_lowerbound_longest_note)
            flag2 = true;
          if (m2.is_bridge(0))
            flag2 = false;
        }
        if (ln0 >= lowerbound_longest_note)
          flag0 = true;
        if (ln1 >= extra_lowerbound_longest_note)
          flag1 = true;
        if (m1.is_bridge(0))
          flag1 = flag2 = false;
        if (m0.is_bridge(0))
        ...
        if (ln0 >= 8 && prev_num % 4 == 0 &&
            (num > 2 || (i < string_result.length() &&
                         string_result[i] >= 'A' && string_result[i] <= 'Z'))) {
          flag0 = true;
          flag1 = flag2 = false;
        }
        if (label == 'x' && ln0 >= 4 && prev_num % 4 == 0) {
          flag0 = true;
          flag1 = flag2 = false;
        }
      ...
      if (num > 0) {
        ...
        if (num >= 4 && label == 'X')
          long_extra.push_back(make_pair(
              new_result.size() - 1,
              original_song->subsection(current_pos - num, current_pos)));
      }
      prev_label = label;
    ...
    for (int i = 0; i < long_extra.size(); ++i) {
      ...
      for (j = i + 1; j < long_extra.size(); ++j) {
        if (new_result[long_extra[i].first].second ==
                new_result[long_extra[j].first].second &&
            contain_match(make_pair(long_extra[i].second, long_extra[j].second),
                          matches)) {
      ...
      if (j < long_extra.size())
        break;
    }

    char current_label = 'A' - 1;
    for (int i = 0; i < new_result.size(); ++i) {
      if (new_result[i].first >= 'A' && new_result[i].first <= 'Z' &&
          new_result[i].first != 'X' && current_label < new_result[i].first) {
        if (current_label + 1 == new_result[i].first)
          current_label++;
        else {
          // switch label
          ...
          for (int j = i; j < new_result.size(); ++j) {
            if (new_result[j].first == label1)
              candidate1.push_back(j);
            else if (new_result[j].first == label2)
    ...
    while (i < new_result.size()) {
      if (i + 1 < new_result.size() && new_result[i].first >= 'A' &&
          new_result[i].first <= 'Z' && new_result[i].first != 'X' &&
          new_result[i].second >= 6 && new_result[i].second < 8 &&
          new_result[i + 1].first == 'X' &&
        ...
        for (int j = i + 2; j + 1 < new_result.size(); ++j) {
          if (new_result[j].first == new_result[i].first &&
              ...
              new_result[j + 1].second + new_result[j].second == 8 &&
              contain_match(make_pair(original_song->subsection(
                                          start_pos[i], start_pos[i] + 8),
                                      original_song->subsection(
                                          start_pos[j], start_pos[j] + 8)),
                            matches)) {
            candidates.push_back(j);
          } else if (new_result[j].first == new_result[i].first &&
                     new_result[j].second <= 5)
            valid_flag = false;
        }
        for (int j = 0; j < i && valid_flag; ++j) {
          if (new_result[j].first == new_result[i].first &&
              new_result[j].second <= 5 && new_result[j + 1].first != 'X')
            valid_flag = false;
        }
    ...
    for (i = 0; i < new_result.size(); ++i) {
      if (new_result[i].first == 'X' &&
          (original_song->subsection(start_pos[i],
                                     start_pos[i] + new_result[i].second))
              .is_bridge()) {
        ...
        } else if (i + 1 < new_result.size() &&
                   new_result[i + 1].first == 'o') {
      ...
      if (new_result[i].second > 0)
        string_result_merge_extra +=
            new_result[i].first + to_string(new_result[i].second);
    }
  }

  double calculate_cost(double g, double h, const string &result) {
    if (result == "")
      return (original_song->end - original_song->start) * g + h;
    int i = 0;
    vector<pair<char, int>> phrases;
    phrases.clear();
    ...
    for (int times = 0; times <= 1; ++times) {
      for (i = phrases.size() - 2; i >= 0; --i) {
        if (phrases[i].first >= 'A' && phrases[i].first <= 'Z' &&
            phrases[i].first != 'X' && phrases[i + 1].first == 'x' &&
            phrases[i + 1].second <= 1) {
          if (i + 2 == phrases.size() ||
              (phrases[i + 2].first >= 'A' && phrases[i + 2].first <= 'Z')) {
            phrases[i].second += phrases[i + 1].second;
    ...
    for (i = 0; i < phrases.size(); i++) {
      if (phrases[i].first >= 'A' && phrases[i].first <= 'Z' &&
      ...
      } else if (phrases[i].first >= 'a' && phrases[i].first <= 'z' &&
                 phrases[i].first != 'x') {
        ...
        if (i == 0 ||
          ...
          new_cost += h;
                 ...
                 phrases[i].second + phrases[i - 1].second > 4)
          new_cost += h;
      }
    }
    ...
    for (i = 0; i < 54; ++i)
      if (counts[i] > 0)
        new_cost +=
            h * double(counts[i]) + g * double(sum[i]) / double(counts[i]);
    return new_cost;
  }
  ...
  bool boundary_checking(string result) {
    ...
    while (i < result.size()) {
      ...
      while (i < result.length() && result[i] >= '0' && result[i] <= '9')
        num = num * 10 + result[i++] - '0';
      if (label >= 'A' && label <= 'Z' && label != 'X' && prev_label >= 'A' &&
          prev_label <= 'Z' && prev_label != 'X' && prev_num % 4 != 0 &&
          prev_label != label) {
        Section m0 =
            original_song->subsection(current_pos - 2, current_pos - 1);
        ...
        if ((ln1 <= (original_song->time_signature) / 2 - 2 &&
             ln2 >= (original_song->time_signature - 4)) ||
            (ln1 <= 4 && ln2 - ln1 >= 4))
          return false;
        if ((ln1 <= (original_song->time_signature) / 2 - 2 &&
             ln0 >= (original_song->time_signature - 4)) ||
            (ln1 <= 4 && ln0 - ln1 >= 4) || (ln1 <= 8 && ln0 >= 14))
          return false;
      }
      prev_label = label;
      prev_num = num;
  ...
  double length_variance(string result) {
    ...
    for (char label = 'A'; label <= max_label; ++label) {
      ...
      double num = 0.0;
      for (i = 0; i < phrases.size(); ++i)
        if (phrases[i].first == label) {
          avg_len += double(phrases[i].second);
          num += 1.0;
        }
      avg_len /= num;
      int std = 4;
      ...
      for (i = 0; i < phrases.size(); ++i)
        if (phrases[i].first == label && phrases[i].second == std &&
            i + 1 < phrases.size() && phrases[i + 1].first != 'o')
          std_appear = true;
      ...
      for (i = 0; i < phrases.size(); ++i)
        if (phrases[i].first == label && phrases[i].second != std) {
          if (phrases[i].second < std) {
            ...
            if (phrases[i].second == std + 1 && std != 12)
              tip = 0;
            if (phrases[i].second == std + 2 && std > 4)
              tip = 0.5;
            if (phrases[i].second == std + 2 && std == 4 &&
                (i + 1 == phrases.size() ||
                 (phrases[i + 1].first >= 'a' && phrases[i + 1].first <= 'z')))
              tip /= 2.0;
            ans += tip * tip;
          }
        }
    ...
    // secessive variance
    for (i = 0; i < phrases.size(); ++i) {
      if (i + 1 < phrases.size() && phrases[i].first >= 'A' &&
          phrases[i].first <= 'Z' && phrases[i].first != 'X' &&
          phrases[i + 1].first >= 'A' && phrases[i + 1].first <= 'Z' &&
          phrases[i + 1].first != 'X' &&
          abs(phrases[i].second - phrases[i + 1].second) >= 7 &&
          (phrases[i].second <= 6 || phrases[i + 1].second <= 6)) {
        if (phrases[i + 1].second <= 6) {
          if (i + 2 >= phrases.size() ||
              phrases[i + 2].first != phrases[i + 1].first)
            ans += 2.0;
        } else {
          if (i == 0 || phrases[i - 1].first != phrases[i].first)
            ans += 2.0;
        }
      }
    }
    double factor = 1.0;
    if (total_length > 60)
      factor = double(total_length) / 60.0;
    return ans * factor;
  }
  ...
  bool is_boundary(int k) {
    ...
    while (i < string_result.size() && t < k) {
      int num = 0;
      i++;
      while (i < string_result.size() && string_result[i] >= '0' &&
             string_result[i] <= '9')
        num = num * 10 + string_result[i++] - '0';
      t += num;
      if (t == k)
...
double chord_dist(Chord x, Chord y) {
  ...
  // similarity between previous chord and next chord, prev_chord_vector of x
  // vs. y
  return f1;
}
...
int get_average_melody(const vector<int> &x) {
  ...
  if (nonzero > 0)
    average_pitch_x = average_pitch_x / nonzero;
  else
    average_pitch_x = 60;
  return average_pitch_x;
}

// Melody Contour Similarity between two melodies using Dynamic Time Warping
// (DTW)
double contour_rating(vector<int> &x, vector<int> &y, int window = 2) {
  int lx = int(x.size()), ly = int(y.size());
  int average_pitch_x = get_average_melody(x),
      average_pitch_y = get_average_melody(y);
  ...
  for (int i = 1; i <= lx; ++i)
    for (int j = max(1, i - window); j <= min(ly, i + window); ++j) {
      dtw[i][j] =
          min3(dtw[i - 1][j - 1] + get_contour_dist(x, i - 1, y, j - 1),
  ...
  for (int i = 0; i < lx; ++i) {
    if (x[i] != -2)
      std += abs(x[i] - average_pitch_x + 2) + 1;
    else
      std += 12;
  }
  for (int i = 0; i < ly; ++i) {
    if (y[i] != -2)
      std += abs(y[i] - average_pitch_y + 2) + 1;
    else
      std += 12;
  }
  std /= 2;
...
pair<double, double> get_begin_melody_score(const vector<int> &x,
                                            const vector<int> &y,
                                            int time_signature) {
...
double melody_dist(vector<int> &x, vector<int> &y, int time_signature,
  ...
  if (is_empty_melody(nnx, 0) && is_empty_melody(nny, 0))
    return 0.0;
  if (is_empty_melody(nx, 0) && is_empty_melody(ny, 0))
    return 0.0;
  tx = is_empty_melody(nnx, 2);
  ty = is_empty_melody(nny, 2);
  if (tx != ty) {
    if (tx && melody_ending_possible(nny) < time_signature * 2 - 4)
      return 0.0;
    if (ty && melody_ending_possible(nnx) < time_signature * 2 - 4)
      return 0.0;
  }
  tx = is_empty_melody(nx, 2);
  ty = is_empty_melody(ny, 2);
  if (tx != ty) {
    if (tx && melody_ending_possible(ny) < time_signature - 6)
      return 0.0;
    if (ty && melody_ending_possible(nx) < time_signature - 6)
      return 0.0;
    if (tx && melody_ending_possible(nny) >= 16)
      return 0.0;
    if (ty && melody_ending_possible(nnx) >= 16)
      return 0.0;
  }
  ...
  if (nr_cr_score1 > cr_score1)
    cr_score1 = nr_cr_score1;
  if ((cr_score0 < 0.45 || cr_score1 < 0.3) && cr_score0 < 0.6 &&
      cr_score1 < 0.6)
    return 0.0;
  ...
  if (length_x >= 12) {
    ...
    if (contour_rating(hx, hy) < 0.5 && contour_rating(nrhx, nrhy) < 0.5)
      return 0.0;
  }

  double cr_score2 = contour_rating(x, y);
  double nr_cr_score2 = contour_rating(nrx, nry);
  if (nr_cr_score2 > cr_score2)
    cr_score2 = nr_cr_score2;
  double tr_cr_score3 = (max(cr_score0, cr_score1) + cr_score2) / 2.0;
  if (length_x >= 10)
    tr_cr_score3 = (max(cr_score0, cr_score1) * 0.5 + cr_score2) / 1.5;
  if (tr_cr_score3 > cr_score2)
    cr_score2 = tr_cr_score3;
...
double cal_melody_score(const Section &a, const Section &b,
                        bool waive_ending = false) {
  vector<int> mela = get_flat_melody(a), melb = get_flat_melody(b);

  double r_score =
      (rhythm_rating(a.melody, b.melody) + rhythm_rating(b.melody, a.melody)) /
      2.0;
  double m_score = (melody_dist(mela, melb, a.time_signature, waive_ending) +
                    melody_dist(melb, mela, a.time_signature, waive_ending)) /
                   2.0;
  int average_a = get_average_melody(mela),
      average_b = get_average_melody(melb);
  ...
  if (abs(average_b - average_a) >= 5) {
    ...
    double tmp_mscore =
        (melody_dist(mela, tmp_melb, a.time_signature, waive_ending) +
         melody_dist(tmp_melb, mela, a.time_signature, waive_ending)) /
        2.0;
    if (tmp_mscore > m_score)
      m_score = tmp_mscore;
  }
  if (m_score * r_score >= 0.6)
  ...
  if (m_score > 0.9) {
    ...
    if (r_score1 * m_score >= 0.6)
      return r_score1 * m_score;
    if (r_score2 * m_score >= 0.6)
      return r_score2 * m_score;
  }
  pair<double, double> mcr_scores =
      get_begin_melody_score(mela, melb, a.time_signature);

  if (r_score > 0.85 && m_score > 0.5 &&
      (mcr_scores.first > 0.5 || mcr_scores.second > 0.5) &&
      !(mcr_scores.first < 0.45 || mcr_scores.second < 0.3))
    return 0.6;
  return m_score * r_score;
}
...
double cal_chord_score(const Section &a, const Section &b) {
  ...
  for (int t = 0; t < len; ++t) {
    ...
    if (i == a.chords.size() || j == b.chords.size())
      break;
    real_len++;
    chord_score += (chord_dist(a.chords[i], b.chords[j]) +
                    chord_dist(b.chords[j], a.chords[i])) /
                   2.0;
  }
  return chord_score / double(real_len) -
         double(abs(a.end - a.start - b.end + b.start)) * 0.03;
}

// Judge if two phrases(fragments) are considered repetition or not
bool approximate_matching(const Section &a, const Section &b,
                          bool waive_ending = false) {
  if (abs(a.end - a.start - b.end + b.start) >= 1)
    return 0; // length
  double chord_score = cal_chord_score(a, b);
  double melody_score = cal_melody_score(a, b, waive_ending);

  if (chord_score >= 0.6 && melody_score >= 0.595)
    return 1;
  if (melody_score > 0.8 && chord_score >= 0.55)
    return 1;
  if (chord_score > 0.9 && melody_score >= 0.55)
    return 1;
  if (melody_score >= 0.95)
    return 1;
...
bool include_matches(pair<Section, Section> candidate,
...
int clear_intro(const Section &song) {
  ...
  if ((real_start % (song.time_signature)) >= song.time_signature - 6)
    real_start = (real_start / song.time_signature) + 1;
  else
    real_start = (real_start / song.time_signature);
  return real_start;
}
...
bool start_boundary_check(const Section &x, const Section &origin) {
  ...
  if (start_measure.is_bridge(4) ||
      origin.is_subsec_bridge(x.start, x.start + 1) ||
      (start_measure.melody[0].pitch == -2 &&
       start_measure.melody[0].duration >= 10))
    return false;
  if (get_longest_note(x.subsection(x.start, x.start + 1).melody, true) < 12)
    return true;
  int beginning_longest =
      get_longest_note(x.subsection(x.start, x.start + 2).melody, true);
  int leftmiddle_longest =
      get_longest_note(x.subsection(x.start + 2, x.end - 1).melody);
  if (x.end - x.start >= 6)
    leftmiddle_longest =
        get_longest_note(x.subsection(x.start + 2, x.end - 2).melody);

  if (leftmiddle_longest <= 8 || leftmiddle_longest + 4 < beginning_longest)
    return false;
  return true;
}
...
vector<pair<Section, Section>> find_all_match(Section song) {
  ...
  for (int i = real_start; i < real_end; ++i) {
    if (i >= real_start + 1 && i <= real_start + 3)
      continue;

    for (int d1 = min(real_end - i, 16); d1 >= 4; --d1) {
      Section x = song.subsection(i, i + d1);
      if (!start_boundary_check(x, song) && !x.is_bridge())
        continue;

      waive_ending_x = false;
      if (i + d1 == real_end || song.subsection(i + d1, i + d1 + 1).is_bridge())
        waive_ending_x = true;
      if (!x.is_bridge() && x.contains_bridge())
        continue;

      for (int j = i + d1; j < real_end; ++j) {
        for (int d2 = real_end - j; d2 >= 4; --d2) {
          Section y = song.subsection(j, j + d2);
          if (!start_boundary_check(y, song) && !y.is_bridge())
            continue;

          waive_ending_y = false;
          if (j + d2 == real_end ||
              song.subsection(j + d2, j + d2 + 1).is_bridge())
            waive_ending_y = true;

          if (!y.is_bridge() && y.contains_bridge())
            continue;
...
bool appearin(const vector<pair<Section, Section>> &matches, const Section &x,
              const Section &y) {
  for (int i = 0; i < matches.size(); ++i)
    if ((matches[i].first.is_same(x) && matches[i].second.is_same(y)) ||
        (matches[i].first.is_same(y) && matches[i].second.is_same(x)))
      return 1;
  return 0;
}

int is_add2clique(const Match &oclique,
                  const vector<pair<Section, Section>> &matches,
                  const pair<Section, Section> &edge) {
  int flag1 = 0, flag2 = 0;
  for (int i = 0; i < oclique.sections.size(); ++i) {
    if (edge.first.is_same(oclique.sections[i]))
      flag1++;
    if (edge.second.is_same(oclique.sections[i]))
      flag2++;
  }
  if (flag1 + flag2 == 0 || flag1 + flag2 >= 2)
    return 0;
  ...
  if (add.start <= oclique.sections.back().start)
    return 0;
...
int find_pair_index(const Section &a, const Section &b,
                    const vector<pair<Section, Section>> &matches,
                    int match_size) {
  for (int i = 0; i < match_size; ++i)
    if ((matches[i].first.start == a.start && matches[i].first.end == a.end &&
         matches[i].second.start == b.start &&
         matches[i].second.end == b.end) ||
        (matches[i].first.start == b.start && matches[i].first.end == b.end &&
         matches[i].second.start == a.start && matches[i].second.end == a.end))
      return i;
  return -1;
}

bool matches_include(const vector<Match> &finals, const Match &current,
                     bool same = false) {
  int finals_len = finals.size();
  int current_len = current.sections.size();
  for (int i = 0; i < finals_len; ++i) {
    if ((!same && current_len <= finals[i].sections.size()) ||
        (same && current_len == finals[i].sections.size())) {
      int sum = 0, k = 0;
...
vector<Match> merge_matches(const vector<pair<Section, Section>> &matches,
                            bool greedy = false) {
  vector<Match> finals;
  ...
  for (int i = 0; i < num; ++i) {
    ...
    if ((matches[i].first.end - matches[i].first.start) == 4 ||
        (matches[i].first.end - matches[i].first.start) % 8 == 0) {
...
Covermap add_cover(Covermap &a, Match x, double g, double h, int real_start,
                   int real_end) {
  Covermap new_map(a);
  int l = 0;
  int delta_seg = 0;
  ...
  for (int idx = 0; idx < section_len; ++idx) {
    ...
    if (s < 64) {
      if (e <= 64)
        new_map.map[0] = new_map.map[0] | (((am << (e - s)) - 1) << s);
      else
        new_map.map[0] = new_map.map[0] | (((am << (64 - s)) - 1) << s);
      s = 64;
    }
    if (s < 128 && e > 64) {
      if (e <= 128)
        new_map.map[1] = new_map.map[1] | (((am << (e - s)) - 1) << (s - 64));
      else
        new_map.map[1] = new_map.map[1] | (((am << (128 - s)) - 1) << (s - 64));
      s = 128;
    }
    if (e > 128)
      new_map.map[2] = new_map.map[2] | (((am << (e - s)) - 1) << (s - 128));
  }
  new_map.cost = new_map.cost - g * double(l) +
                 g * double(l) / double(x.sections.size()) +
                 h * double(delta_seg);
  new_map.cover = x;
  new_map.from = &a;
...
void map_update(vector<Covermap> &x, Covermap const &y) {
  ...
  for (int i = 0; i < xlen; ++i) {
    if (x[i].is_same(y)) {
      flag = 1;
      if (x[i].cost > y.cost ||
          (fabs(x[i].cost - y.cost) < 1e-5 && x[i].match_score < y.match_score))
        x[i] = y;
      break;
    }
  }
...
bool cmp_match(const Match &a, const Match &b) {
  return (a.sections.size() * (a.sections[0].end - a.sections[0].start) >
              b.sections.size() * (b.sections[0].end - b.sections[0].start) ||
          (a.sections.size() * (a.sections[0].end - a.sections[0].start) ==
               b.sections.size() * (b.sections[0].end - b.sections[0].start) &&
           a.score > b.score));
}
...
bool boundary_cmp(pair<int, double> &a, pair<int, double> &b) {
  return (a.second > b.second + 1e-6 ||
          (a.second > b.second - 1e-6 && a.first < b.first));
}
...
Covermap dp_max_cover(Section song, vector<Match> match,
                      const vector<Match> &original_match,
                      const vector<pair<Section, Section>> &matches, double g,
                      double h, double tmp_bcost = 1000000000.0,
                      bool add_on = false) {
  int n = match.size();
  ...
  if (tmp_bcost > dp[0][0].cost)
    tmp_bcost = dp[0][0].cost;
  ...
  for (int i = 0; i < n; ++i) {
    ...
    /* IMPORTANT! This is for fast testing: ff the algorithm is running more
     * than 20 minutes, then quit.   */
    ...
    if (dp[i].size() > max_covermap_usage)
      max_covermap_usage = dp[i].size();
    ...
    while (j < lensize) {
      if ((dp[i + 1][j].best_possible_cost(g, h, match[i].sections[0].start,
                                           real_end) -
           tmp_bcost) > tolerance) {
    ...
    for (vector<Covermap>::iterator status = dp[i].begin();
         status != dp[i].end(); ++status) {
      if (status->is_ok2add_cover(match[i])) {
        Covermap new_status =
            add_cover(*status, match[i], g, h, real_start, real_end);
        if (new_status.cost < tmp_bcost + tolerance ||
            new_status.best_possible_cost(g, h, match[i].sections[0].start,
                                          real_end) < tmp_bcost + tolerance)
  ...
  for (int j = 0; j < final_len; ++j) {
    ...
    if (dp[n][j].cost < best_before_merge_cost &&
        dp[n][j].boundary_checking(dp[n][j].string_result))
      best_before_merge_cost = dp[n][j].cost;

    dp[n][j].merge_extra(matches, true);
    double alternative_result =
        dp[n][j].calculate_cost(g, h, dp[n][j].string_result_merge_extra);
    ...
    cover_area.push_back(make_pair(
        dp[n][j].calculate_cover_area(dp[n][j].string_result_merge_extra),
        dp[n][j].calculate_cover_area(dp[n][j].string_result)));

    if (dp[n][j].cost < bcost &&
        dp[n][j].boundary_checking(dp[n][j].string_result_merge_extra)) {
      bcost = dp[n][j].cost;
      best = j;
    }
  }
  ...
  for (int j = 0; j < final_len; ++j)
    if (dp[n][j].boundary_checking(dp[n][j].string_result_merge_extra) &&
        (before_merge_cost[j] - best_before_merge_cost < tolerance ||
         (before_merge_cost[j] - best_before_merge_cost < tolerance * 2.2 &&
          dp[n][j].cost - tolerance_bcost < tolerance))) {
      bool add_flag = true;
      for (int t = 0; t < candidates.size(); ++t) {
        if (dp[n][j].string_result_merge_extra ==
            dp[n][candidates[t]].string_result_merge_extra) {
          add_flag = false;
          if (before_merge_cost[j] + 1e-5 < before_merge_cost[candidates[t]] ||
              (fabs(before_merge_cost[j] - before_merge_cost[candidates[t]]) <
                   1e-5 &&
  ...
  for (int t = 0; t < candidates.size(); ++t) {
    int j = candidates[t];
    if (cover_area[j].first > best_cover.first)
      best_cover.first = cover_area[j].first;
    if (cover_area[j].second > best_cover.second)
      best_cover.second = cover_area[j].second;
  }
  for (int t = 0; t < candidates.size(); ++t) {
    int j = candidates[t];
    int l0 = dp[n][j].string_result.length(),
        l1 = dp[n][j].string_result_merge_extra.length();
    if (best_cover.first > cover_area[j].first &&
        (dp[n][j].string_result_merge_extra[l1 - 2] == 'X' ||
         (dp[n][j].string_result_merge_extra[l1 - 2] == 'o' &&
          dp[n][j].string_result_merge_extra[l1 - 4] == 'X')))
      cover_area[j].first++;
    if (best_cover.second > cover_area[j].second &&
        (dp[n][j].string_result[l0 - 2] == 'X' ||
         (dp[n][j].string_result[l0 - 2] == 'o' &&
          dp[n][j].string_result[l0 - 4] == 'X')))
      cover_area[j].second++;
  }
  ...
  for (int t = 0; t < candidates.size(); ++t) {
    int j = candidates[t];
    double lenvar =
        dp[n][j].length_variance(dp[n][j].string_result_merge_extra);
  ...
  for (int t = 0; t < candidates.size(); ++t) {
    ...
    double lenvar =
        dp[n][j].length_variance(dp[n][j].string_result_merge_extra);
    double cover_score = double(best_cover.first - cover_area[j].first +
                                best_cover.second - cover_area[j].second) /
                         8.0;
    if (fabs(before_merge_cost[j] - best_before_merge_cost) < 1e-5 &&
        fabs(dp[n][j].cost - tolerance_bcost) < 1e-5 &&
        fabs(dp[n][j].cost + before_merge_cost[j] + lenvar + cover_score -
             bcost) < 1e-5) {
  ...
  if (best != -1) {
    ...
    for (int t = 0; t < candidates.size(); ++t) {
      ...
      double lenvar =
          dp[n][j].length_variance(dp[n][j].string_result_merge_extra);
      double cover_score =
          0.5 * double(best_cover.first - cover_area[j].first) +
          0.1 * double(best_cover.second - cover_area[j].second);
      double cur =
          dp[n][j].cost + 0.95 * before_merge_cost[j] + lenvar + cover_score;
      if (cur < bcost ||
          (fabs(cur - bcost) < 1e-5 &&
           dp[n][j].match_score > dp[n][best].match_score) ||
          ((fabs(cur - bcost) < 1e-5 &&
            fabs(dp[n][j].match_score - dp[n][best].match_score) < 1e-5) &&
           ((cover_area[j].first > cover_area[best].first &&
             cover_area[j].second + 2 >= cover_area[best].second) ||
            (cover_area[j].first == cover_area[best].first &&
             cover_area[j].second > cover_area[best].second) ||
            (cover_area[j].first == cover_area[best].first &&
             cover_area[j].second == cover_area[best].second &&
  ...
  for (vector<Match>::iterator mj = tmp_original_match.begin();
       mj != tmp_original_match.end(); ++mj) {
    if (dp[n][best].is_ok2add_cover(*mj)) {
      Covermap new_status =
          add_cover(dp[n][best], *mj, g, h, real_start, real_end);
      new_status.toString();
      new_status.merge_extra(matches);
      new_status.cost =
          new_status.calculate_cost(g, h, new_status.string_result_merge_extra);
...
bool groundtruth_cmp(string gt, string result) {
  ...
  while (outtro_pos >= 0 && result[outtro_pos] >= '0' &&
         result[outtro_pos] <= '9')
    outtro_pos--;
  int num = 0;
...
int main(int argc, char *argv[]) {
  ...
  // load data
  FILE *inm, *inc, *ink;
  inm = fopen((dir_path + "/melody.txt").c_str(), "r");
  inc = fopen((dir_path + "/finalized_chord.txt").c_str(), "r");
  ink = fopen((dir_path + "/analyzed_key.txt").c_str(), "r");
  ...
  if (melody_length > song.end)
    song.end = melody_length;
  ...
  out = fopen((output_dir + "/phrases_log.txt").c_str(), "w");
  outseg = fopen((output_dir + "/phrases.txt").c_str(), "w");

  // matching
  int real_end = clear_outtro(song);
  int real_start = clear_intro(song);
  int real_len = real_end - real_start;
  fprintf(out, "song %s, real len: %d\n", dir_path.c_str(), real_len);
  ...
  if (matches.size() == 0) {
    ...
    string temp_result = "";
    if (real_start > 0)
      temp_result += "i" + to_string(real_start);
    temp_result += "X" + to_string(real_len);
    if (real_end < song.end)
      temp_result += "o" + to_string(song.end - real_end);
    fprintf(outseg, "%s\n", temp_result.c_str());
    fprintf(out, "%s\n", temp_result.c_str());
  ...
  /* IMPORTANT! this is an approximation for fast testing: when the matches are
   * too much, reduce them */
  ...
  int percent = 20;
  if (greedy_size >= 600 || real_len >= 80)
    percent = 10;

  // Approximation version 1: only use the top 20% or 10% of the match sets by
  // matching score
  vector<Match> final_match_maximum;
  int last_temp_size = 0;
  for (int tcount = 0; tcount < 20; ++tcount) {
    int temp_size =
        max(int(greedy_size * (tcount + 1) / 20), last_temp_size + 1);
    if (temp_size > greedy_size)
      temp_size = greedy_size;
    if (temp_size > last_temp_size) {
      vector<Match> temp(final_match_greedy.begin() + last_temp_size,
                         final_match_greedy.begin() + temp_size);
      sort(temp.begin(), temp.end(), cmp_maximum_match);
      final_match_maximum.insert(final_match_maximum.end(), temp.begin(),
                                 temp.begin() +
                                     int(greedy_size * percent / 2000) + 1);
      last_temp_size = temp_size;
    }
  }
  sort(final_match_maximum.begin(), final_match_maximum.end(), cmp_order_match);
  Covermap *maximum_result =
      new Covermap(dp_max_cover(song, final_match_maximum, final_match_greedy,
                                matches, g, h, 1000000.0, true));
  fprintf(out, "MAXIMUM %d percent: %s cost %.2lf match_score %.2lf\n", percent,
          maximum_result->string_result_merge_extra.c_str(),
          maximum_result->cost, maximum_result->match_score);

  // Approximation version 2: only use the top 20% or 10% of the match sets by
  // covering length
  ...
  for (int tcount = 0; tcount < 20; ++tcount) {
    ...
    if (temp_size > last_temp_size) {
      ...
      final_match_appro.insert(final_match_appro.end(), temp.begin(),
                               temp.begin() +
                                   int(greedy_size * percent / 2000) + 1);
      last_temp_size = temp_size;
    }
  ...
  Covermap *appro_result =
      new Covermap(dp_max_cover(song, final_match_appro, final_match_greedy,
                                matches, g, h, 1000000.0, true));
  fprintf(out, "APPROX  %d percent: %s cost %.2lf match_score %.2lf\n", percent,
          appro_result->string_result_merge_extra.c_str(), appro_result->cost,
          appro_result->match_score);

  // Approximation version 3: use the combination of partial match sets in
  // approximation version 1 & 2
  vector<Match> final_match_mix;
  final_match_mix.insert(final_match_mix.end(), final_match_maximum.begin(),
                         final_match_maximum.end());
  final_match_mix.insert(final_match_mix.end(), final_match_appro.begin(),
                         final_match_appro.end());
  ...
  Covermap *mix_result =
      new Covermap(dp_max_cover(song, final_match_mix, final_match_greedy,
                                matches, g, h, 1000000.0, true));
  fprintf(out, "MIX-MA  %d percent: %s cost %.2lf match_score %.2lf\n", percent,
          mix_result->string_result_merge_extra.c_str(), mix_result->cost,
          mix_result->match_score);
  ...
  for (int j = 0; j < random_num; ++j) {
    ...
    if (greedy_size > 0)
      shuffle(final_match_greedy.begin(), final_match_greedy.end(), gen);
    vector<Match> final_match_random(
        final_match_greedy.begin(),
        final_match_greedy.begin() +
            int(greedy_size /* * (double(0.5) * (j / (random_num / 2) + 2))*/ /
                10) +
            1);
    if (greedy_size > 0)
      sort(final_match_random.begin(), final_match_random.end(),
           cmp_order_match);
    Covermap *tmp_random_result =
        new Covermap(dp_max_cover(song, final_match_random, final_match_greedy,
                                  matches, g, h, 1000000.0, true));
    if (random_result == NULL ||
        random_result->cost > tmp_random_result->cost ||
        (random_result->cost > tmp_random_result->cost - 1e-6 &&
         random_result->match_score < tmp_random_result->match_score))
      random_result = tmp_random_result;
  }
  fprintf(out, "RANDOM %d percent: %s cost %.2lf match_score %.2lf\n", 10,
          random_result->string_result_merge_extra.c_str(), random_result->cost,
          random_result->match_score);
  endTime = clock();
  double runTime = (double)(endTime - startTime) / CLOCKS_PER_SEC;
  fprintf(out, "Four approximations time used in total: %.2lf\n", runTime);

  // Approximation version 5: reducing the match sets by deleting invalid
  // boundary matches
  //                          according to previous approximations
  vector<int> boundaries;
  boundaries.push_back(song.start);
  for (int k = song.start + 1; k < song.end; ++k) {
    if (appro_result->is_boundary(k) && maximum_result->is_boundary(k) &&
        random_result->is_boundary(k))
  ...
  while (j < lensize) {
    ...
    for (int k = 1; k < boundaries.size() - 1; ++k)
      if ((matches[j].first.start < boundaries[k] &&
           matches[j].first.end > boundaries[k]) ||
          (matches[j].second.start < boundaries[k] &&
           matches[j].second.end > boundaries[k])) {
  ...
  // FULL version: structure segment analysis => dynamic programming + A*, set g
  // & h
  if (greedy_size > 0)
    sort(final_match_greedy.begin(), final_match_greedy.end(), cmp_order_match);
  // if (greedy_size > 0) sort(final_match_greedy_after_cut.begin(),
  // final_match_greedy_after_cut.end(), cmp_order_match);

  Covermap *result =
      new Covermap(dp_max_cover(song, final_match_greedy, final_match_greedy,
                                matches, g, h, best_approach->cost + 10.0));
  if (result->cost < 1000000000.0) {
    fprintf(out, "FINAL : ");
    best_approach = result;
  } else
    fprintf(out, "Use approximation : ");

  fprintf(outseg, "%s\ncost %.2lf match_score %.2lf\n",
          best_approach->string_result_merge_extra.c_str(), best_approach->cost,
          best_approach->match_score);
